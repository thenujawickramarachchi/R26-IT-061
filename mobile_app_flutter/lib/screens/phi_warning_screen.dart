...
class _PHIWarningScreenState extends State<PHIWarningScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final Map<String, dynamic>? email = result?['email'] is Map
        ? Map<String, dynamic>.from(result!['email'] as Map)
        : null;

    // The backend returns both a top-level `email_sent` flag and a
    // nested `email.sent` flag. Accept either one so the UI always
    // reflects the actual delivery result returned by /send-warning.
    final bool emailSent =
        _asBool(result?['email_sent']) || _asBool(email?['sent']);

    final bool phiOfficerFound = result?['phi_officer_found'] == null
        ? true
        : _asBool(result?['phi_officer_found']);

    final String emailMessage = (email?['message'] ?? '').toString();
    final String recipient = (email?['recipient'] ?? '').toString();
    final bool noPhiOfficer = !phiOfficerFound;
              ...
              _statusCard(
                ...
                message: emailSent
                    ...
                    ? recipient.isNotEmpty
                        ...
                        ? 'High-risk warning email was sent successfully to $recipient.'

                        : (emailMessage.isNotEmpty

                            ? emailMessage

                            : 'High-risk warning email was sent successfully.')
                    ...
                    : noPhiOfficer
                        ...
                        : emailMessage.isEmpty
                            ...
                            ? 'The alert was saved, but email delivery was not confirmed.'
